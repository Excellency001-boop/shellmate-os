...
export default function ShellmatesOS() {
  ...
  const [logs, setLogs] = useState<string[]>(["SYSTEM BOOT COMPLETE", "TMA PROTOCOL LOADED..."]);
  ...
  const triggerScan = () => {
    setIsScanning(true);
    pushLog("SCANNING DEPIN NETWORK...");
    setTimeout(() => {
      const bonus = Math.floor(Math.random() * 500 * active.mult);
      setXp(v => v + bonus);
      pushLog(`FRAGMENT SYNCED: +${bonus} XP`);
      setIsScanning(false);
    }, 1500);
  };

  const shareToX = () => {
    const text = `Syncing my @Shellmates OS Profile: ${active.label} 🐢\nXP Score: ${xp}\n\nBuilt by @Ayo_xtt #Shellmates #Web3`;
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-zinc-900/40 border border-white/5 rounded-[40px] p-8 aspect-square flex flex-col items-center justify-center relative overflow-hidden backdrop-blur-xl">
            <div className="absolute top-8 left-8 flex items-center gap-2">
              ...
              <span className="text-[10px] tracking-widest text-zinc-500 uppercase">{sync ? 'Neural Link Active' : 'Offline'}</span>
            </div>
            ...
            <div className="absolute bottom-8 right-8">
              <p className={`text-[10px] font-black px-4 py-2 rounded-full border border-white/10 bg-black/60 backdrop-blur-md uppercase ${active.color}`}>
                {active.rarity} // MULT: {active.mult}x
              </p>
            </div>
          </div>

          <div className="bg-black/60 border border-white/5 p-6 rounded-2xl h-32 overflow-hidden shadow-inner">
            {logs.map((l, i) => (
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="flex justify-between items-end border-b border-white/5 pb-6">
            <div>
              ...
              <p className="text-[10px] text-zinc-500 mt-2 tracking-[0.4em]">NEURAL INTERFACE v2.6</p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <button 
              onClick={() => { setSync(!sync); pushLog(sync ? "DISCONNECTING..." : "SYNCING BRAINWAVES..."); }}
              className={`py-6 rounded-2xl font-black uppercase text-xs transition-all ${sync ? 'bg-blue-600 shadow-[0_0_50px_rgba(37,99,235,0.3)] border-blue-400' : 'bg-zinc-900 text-zinc-500 border border-white/5'}`}
            >
              {sync ? '[ LINK ACTIVE ]' : '[ INITIALIZE SYNC ]'}
            </button>
            <button 
              disabled={!sync || isScanning}
              onClick={triggerScan}
              className="bg-white text-black py-6 rounded-2xl font-black uppercase text-xs disabled:opacity-20 transition-all hover:bg-blue-500 hover:text-white"
            >
              {isScanning ? 'Processing...' : 'Neural Scan'}
            </button>
          </div>

          {/* THE GRID OF TRAITS (NOW WITH TMA) */}
          <div className="grid grid-cols-3 lg:grid-cols-4 gap-2">
            {Object.entries(TRAITS).map(([id, data]) => (
              <button 
                key={id}
                onClick={() => { setTrait(id); pushLog(`SWITCHED TO ${data.label.toUpperCase()}`); setXp(v => v + 15); }}
                className={`p-3 rounded-xl text-[9px] font-black border transition-all ${trait === id ? 'bg-zinc-100 text-black border-white' : 'bg-zinc-900/50 border-white/5 text-zinc-500 hover:border-white/20'}`}
              >
                {data.label}
              </button>
            ))}
          ...
          {sync && (
            <button onClick={shareToX} className="w-full py-5 bg-[#1DA1F2] rounded-2xl font-black uppercase text-xs hover:shadow-[0_0_30px_rgba(29,161,242,0.4)] transition-all">
